...
void AGameGrid::BeginPlay()
{
	...
	for (int i = 0; i < NumRows; i++)
	{
		...
		{
			AGameSlot* GameSlot =
				Cast<AGameSlot>(GridActors[GridIndex]->GetChildActor());
...
void AGameGrid::OnConstruction(const FTransform& Transform)
{
	...
	for (int i = 0; i < NumRows; i++)
	{
		...
		{
			FName GridName(
				FString::Printf(TEXT("Slot%dx%d"), j, i));

			auto Grid = NewObject<UChildActorComponent>(
				this,
			...
			Grid->AttachToComponent(
				RootComponent,
				FAttachmentTransformRules::KeepRelativeTransform);

			Grid->SetRelativeLocation(
				FVector(
					(NumRows - i - 1) * Extends.X -
					(NumRows * .5f - .5f) * Extends.X,
					j * Extends.Y -
					(NumCols * .5f - .5f) * Extends.Y,
					0));
			...
			AGameSlot* GameSlot =
				Cast<AGameSlot>(Grid->GetChildActor());
